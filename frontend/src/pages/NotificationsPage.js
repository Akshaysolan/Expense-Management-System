...
function getMeta(type) { return TYPE_META[type] || TYPE_META['info']; }
    ...
    >
      ...
      <div className="notif-body">
        <div className="notif-header-row">
          <span className="notif-type-tag" style={{ color: meta.color, background: meta.bg }}>{meta.label}</span>
      ...
      <div className="notif-actions">
        {!notif.is_read && (
          <motion.button className="notif-btn notif-btn--read" onClick={() => onMarkRead(notif.id)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} title="Mark as read">
            <Check size={14} />
          </motion.button>
        )}
        <motion.button className="notif-btn notif-btn--delete" onClick={() => onDelete(notif.id)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} title="Delete">
          <Trash2 size={14} />
        </motion.button>
      </div>
    </motion.div>
...
function NotificationsPage() {
  ...
  const visible = notifications.filter(n => {
    const matchesRead = filter === 'all' ? true : filter === 'unread' ? !n.is_read : n.is_read;
    ...
    <div className="notif-page">
      ...
      <div className="notif-page__header">
        ...
        <div className="notif-header-actions">
          <button className="notif-hdr-btn" onClick={fetchNotifications}><RefreshCw size={14} /> Refresh</button>
          {unreadCount > 0 && <button className="notif-hdr-btn notif-hdr-btn--primary" onClick={markAllRead}><CheckCheck size={14} /> Mark all read</button>}
          {notifications.length > 0 && <button className="notif-hdr-btn notif-hdr-btn--danger" onClick={clearAll}><Trash2 size={14} /> Clear all</button>}
        </div>
      </div>
      ...
      <div className="notif-filters">
        ...
        <div className="notif-type-wrap" ref={typeMenuRef}>
          <button className="notif-type-btn" onClick={() => setShowTypeMenu(v => !v)}>
            <Filter size={13} />
            {typeFilter === 'all' ? 'All Types' : typeFilter.replace(/_/g, ' ')}
            <ChevronDown size={13} className={`notif-chevron ${showTypeMenu ? 'notif-chevron--open' : ''}`} />
          </button>
          <AnimatePresence>
            {showTypeMenu && (
              <motion.div className="notif-type-menu" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15 }}>
                {allTypes.map(t => (
                  <div key={t} className={`notif-type-opt ${typeFilter === t ? 'active' : ''}`} onClick={() => { setTypeFilter(t); setShowTypeMenu(false); }}>
                    <span className="notif-type-opt__label">{t === 'all' ? 'All Types' : t.replace(/_/g, ' ')}</span>
                    <span className="notif-type-opt__count">{t === 'all' ? notifications.length : notifications.filter(n => n.notification_type === t).length}</span>
                  </div>
                ))}
              </motion.div>
            )}
      ...
      {loading ? (
        ...
        <div className="notif-empty">
          ...
          <button className="notif-hdr-btn notif-retry-btn" onClick={fetchNotifications}><RefreshCw size={14} /> Try again</button>
        </div>
      ) : visible.length === 0 ? (
        <div className="notif-empty">
          <div className="notif-empty__icon"><Bell size={28} /></div>
          <h3>{filter === 'unread' ? 'No unread notifications' : 'No notifications'}</h3>
          <p>{filter === 'unread' ? "You've read everything. Great job!" : 'Notifications will appear here.'}</p>
        </div>
      ) : (
        <AnimatePresence>
          {Object.entries(groups).map(([label, items]) => (
            <div key={label}>
              ...
              <AnimatePresence>
                {items.map(n => <NotifCard key={n.id} notif={n} onMarkRead={markRead} onDelete={deleteNotif} />)}
              </AnimatePresence>
            </div>
          ))}
        </AnimatePresence>
