...
function PDFAnalyticsPage() {
  ...
  if (error) return (
    <div className="pdf-analytics-error">
      ...
      <p>{error}</p>
      <button className="retry-btn" onClick={handleRefresh}><RefreshCw size={16} /> Retry</button>
    </div>
  );

  /* History View */
  if (!id && pdfData?.history) return (
          ...
          {pdfData.history.map((pdf) => (
            ...
            >
              ...
              <div className="stats-preview">
                ...
                <div className="stat-item">
                  <div className="stat-value">{formatCurrency(pdf.total_amount || 0)}</div>
                  <div className="stat-label">Total</div>
                </div>
              </div>
              <div className="card-footer">
                <span className="date-badge"><Calendar size={12} />{new Date(pdf.uploaded_at).toLocaleDateString()}</span>
                <button className="view-btn">View Analytics <ChevronRight size={14} /></button>
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <div className="pdf-analytics-page">
      ...
      <div className="stats-grid">
        {[
          ...
          { icon: <DollarSign size={22} />, bg: '#d1fae5', color: '#065f46', label: 'Total Amount', value: formatCurrency(pdfData?.total_amount || 0) },
          { icon: <Tag size={22} />, bg: '#fee2e2', color: '#991b1b', label: 'Categories', value: pdfData?.unique_categories || 0 },
          { icon: <Clock size={22} />, bg: '#fef3c7', color: '#92400e', label: 'Upload Date', value: pdfData?.uploaded_at ? new Date(pdfData.uploaded_at).toLocaleDateString() : 'N/A' },
        ].map(({ icon, bg, color, label, value }) => (
          <div className="stat-card" key={label}>
            <div className="stat-icon" style={{ background: bg, color }}>{icon}</div>
            <div className="stat-content"><h3>{label}</h3><p>{value}</p></div>
          </div>
        ))}
      </div>
        ...
        <div className="chart-header">
          ...
          <div className="chart-controls">
            {['bar', 'pie', 'line'].map((type) => (
              <button key={type} className={`chart-control-btn ${chartType === type ? 'active' : ''}`} onClick={() => setChartType(type)}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'bar' ? (
              <BarChart data={pdfData?.category_breakdown || []}>
                ...
                <YAxis tick={{ fontSize: 12, fill: 'var(--text-faint)' }} />
                <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 8, color: 'var(--text-primary)' }} />
                <Legend />
                <Bar dataKey="amount" fill="var(--brand-primary)" radius={[4, 4, 0, 0]} />
              </BarChart>
            ) : chartType === 'pie' ? (
              <RePieChart>
                <Pie data={pdfData?.category_breakdown || []} cx="50%" cy="50%" labelLine={false} label={({ name, amount }) => `${name}: ${formatCurrency(amount)}`} outerRadius={150} dataKey="amount">
                  {(pdfData?.category_breakdown || []).map((_, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 8 }} />
              </RePieChart>
            ) : (
              <LineChart data={pdfData?.monthly_trends || []}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                <XAxis dataKey="month" tick={{ fontSize: 12, fill: 'var(--text-faint)' }} />
                <YAxis tick={{ fontSize: 12, fill: 'var(--text-faint)' }} />
                <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 8 }} />
                <Legend />
                <Line type="monotone" dataKey="amount" stroke="var(--brand-primary)" strokeWidth={2} dot={{ r: 4, fill: 'var(--brand-primary)' }} activeDot={{ r: 6 }} />
              </LineChart>
            )}
          </ResponsiveContainer>
        </div>
      ...
      <div className="expenses-table">
        <div className="table-header">
          ...
          <button className="export-btn"><Download size={15} /> Export CSV</button>
        </div>
