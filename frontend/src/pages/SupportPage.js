...
const api = {
  getFAQs:            () => fetch(`${API_BASE_URL}/faqs/`).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  getDocuments:       () => fetch(`${API_BASE_URL}/documents/`).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  getStats:           () => fetch(`${API_BASE_URL}/stats/`).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  submitTicket:       (d) => fetch(`${API_BASE_URL}/tickets/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(d) }).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  createChatSession:  (d) => fetch(`${API_BASE_URL}/chat/sessions/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(d) }).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  sendChatMessage:    (id, msg) => fetch(`${API_BASE_URL}/chat/sessions/${id}/send/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: msg }) }).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  getChatMessages:    (id) => fetch(`${API_BASE_URL}/chat/sessions/${id}/messages/`).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
  closeChatSession:   (id) => fetch(`${API_BASE_URL}/chat/sessions/${id}/close/`, { method: 'POST' }).then(r => { if (!r.ok) throw new Error(); return r.json(); }),
...
function SupportPage() {
  ...
  const [loading, setLoading]     = useState({ faqs: true, documents: true, stats: true });
  ...
  const loadInitialData = async () => {
    try {
      const [faqsData, docsData, statsData] = await Promise.all([api.getFAQs(), api.getDocuments(), api.getStats()]);
      setFaqs(faqsData); setDocuments(docsData); setStats(statsData);
    } catch (err) {
  ...
  const filteredFaqs = faqs.filter(f => {
    ...
    const matchFilter = faqFilter === 'All' || f.tag === faqFilter;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: `Ticket #${ticket.ticket_id} created!` });
      setTimeout(() => setFormSuccess(false), 5000);
    } catch { setToast({ type: 'error', message: 'Failed to submit ticket. Please try again.' }); }
  ...
  const startChat = async () => {
    setChatLoading(true);
    try {
      const session = await api.createChatSession({ name: 'Guest User', email: 'guest@example.com' });
      setChatSession(session); setChatMessages(session.messages || []); setChatOpen(true);
    } catch { setToast({ type: 'error', message: 'Failed to start chat. Please try again.' }); }
  ...
  const sendChatMessage = async () => {
    ...
    setChatTyping(true);
    try {
      await api.sendChatMessage(chatSession.session_id, userMsg.text);
      setTimeout(() => {
        const replies = [
        ...
        setChatMessages(prev => [...prev, { from: 'agent', text: replies[Math.floor(Math.random() * replies.length)], time: 'now' }]);
        setChatTyping(false);
      }, 1400);
  ...
  if (error && !faqs.length) return (
    <div className="sp-error-state">
      ...
      <p>{error}</p>
      <button onClick={loadInitialData} className="sp-btn-primary">Try Again</button>
    </div>
  );
    ...
    <div className="sp-page">
      ...
      <motion.div className="sp-header" initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
        <div className="sp-header__text">
          ...
          <p className="sp-header__sub">Find answers, read documentation, or reach our team directly.</p>
        </div>
        <nav className="sp-quicknav">
          {[{ id: 'cards', label: 'Overview' }, { id: 'stats', label: 'Stats' }, { id: 'faq', label: 'FAQs' }, { id: 'docs', label: 'Docs' }, { id: 'contact', label: 'Contact' }].map(n => (
            <button key={n.id} className={`sp-quicknav__btn ${activeSection === n.id ? 'active' : ''}`} onClick={() => scrollTo(n.id)}>{n.label}</button>
          ))}
        </nav>
      </motion.div>
      ...
      <motion.div id="cards" className="sp-cards" variants={stagger} initial="hidden" animate="show">
        {[
          ...
          { icon: <BookOpen size={26} />, color: '#60a5fa', title: 'Documentation', desc: 'In-depth guides and user manuals', action: 'View Docs', onClick: () => scrollTo('docs') },
      ...
      <section id="faq" className="sp-section">
        ...
        <div className="sp-faq-toolbar">
          <div className="sp-faq-search">
            ...
            <input type="text" placeholder="Search FAQs…" value={faqSearch} onChange={e => setFaqSearch(e.target.value)} className="sp-faq-search__input" />
            {faqSearch && <button className="sp-faq-search__clear" onClick={() => setFaqSearch('')}><X size={14} /></button>}
          </div>
          <div className="sp-faq-filters">
            {faqTags.map(tag => <button key={tag} className={`sp-faq-tag ${faqFilter === tag ? 'active' : ''}`} onClick={() => setFaqFilter(tag)}>{tag}</button>)}
          </div>
        </div>
        <div className="sp-faq-list">
          {loading.faqs ? <div className="sp-loading">Loading FAQs...</div> : (
            <AnimatePresence>
              {filteredFaqs.length === 0
                ? <motion.div className="sp-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><HelpCircle size={32} /><p>No FAQs match your search.</p></motion.div>
                : filteredFaqs.map((faq, index) => (
                  <motion.div key={faq.id || index} className={`sp-faq-item ${activeFaq === index ? 'open' : ''}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
                    <button className="sp-faq-q" onClick={() => setActiveFaq(activeFaq === index ? null : index)} aria-expanded={activeFaq === index}>
                      <span className="sp-faq-q__tag">{faq.tag}</span>
                      <span className="sp-faq-q__text">{faq.question}</span>
                      <motion.span className="sp-faq-q__chevron" animate={{ rotate: activeFaq === index ? 180 : 0 }} transition={{ duration: 0.25 }}><ChevronDown size={18} /></motion.span>
                    </button>
                    <AnimatePresence>
                      {activeFaq === index && (
                        <motion.div className="sp-faq-a" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.28, ease: 'easeInOut' }}>
      ...
      <section id="docs" className="sp-section">
        ...
        <motion.div className="sp-docs-grid" variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}>
          {loading.documents ? <div className="sp-loading">Loading documents...</div> : (
            documents.map((doc, i) => (
              <motion.a key={doc.id || i} href={doc.external_url || '#'} className="sp-doc-card" variants={fadeUp} whileHover={{ y: -4 }} target="_blank" rel="noopener noreferrer">
        ...
        <div className="sp-contact-opts">
          {[
            ...
            <motion.div key={i} className="sp-contact-opt" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ '--opt-color': opt.color }}>
              ...
              <div className="sp-contact-opt__body"><h4>{opt.label}</h4><p>{opt.sub}</p></div>
              <button className="sp-contact-opt__btn" onClick={opt.onClick} disabled={opt.loading}>
                {opt.loading ? <Loader size={14} className="sp-spinner" /> : opt.action}
              </button>
            </motion.div>
          ))}
        </div>
        <motion.div className="sp-form-wrap" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          <div className="sp-form-header">
            <h3 className="sp-form-title">Send us a message</h3>
            <p className="sp-form-sub"><Clock size={13} /> Typical response within 24 hours</p>
          </div>
          <AnimatePresence mode="wait">
            {formSuccess ? (
              <motion.div className="sp-form-success" key="success" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }}>
                <div className="sp-form-success__icon"><CheckCircle size={40} /></div>
                <h4>Message Sent!</h4>
                <p>Thank you for reaching out. Our team will respond within 24 hours.</p>
                <button className="sp-btn-primary" onClick={() => setFormSuccess(false)}>Send Another Message</button>
              </motion.div>
            ) : (
              <motion.form key="form" className="sp-form" onSubmit={handleSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} noValidate>
                <div className="sp-form-row">
                  <div className={`sp-field ${formErrors.name ? 'error' : ''}`}>
                    <label className="sp-field__label">Full Name <span>*</span></label>
                    <input type="text" name="name" className="sp-field__input" placeholder="John Smith" value={contactForm.name} onChange={handleInputChange} disabled={formLoading} />
                  ...
                  <div className={`sp-field ${formErrors.email ? 'error' : ''}`}>
                    ...
                    <input type="email" name="email" className="sp-field__input" placeholder="you@company.com" value={contactForm.email} onChange={handleInputChange} disabled={formLoading} />
                ...
                <div className="sp-form-row">
                  <div className={`sp-field ${formErrors.subject ? 'error' : ''}`}>
                    ...
                    <input type="text" name="subject" className="sp-field__input" placeholder="e.g. Expense submission issue" value={contactForm.subject} onChange={handleInputChange} disabled={formLoading} />
                  ...
                  <div className="sp-field">
                    ...
                    <select name="priority" className="sp-field__input" value={contactForm.priority} onChange={handleInputChange} disabled={formLoading}>
                ...
                <div className={`sp-field ${formErrors.message ? 'error' : ''}`}>
                  <label className="sp-field__label">Message <span>*</span><span className="sp-field__count">{contactForm.message.length} / 1000</span></label>
                  <textarea name="message" className="sp-field__input sp-field__textarea" placeholder="Describe your issue in detail." value={contactForm.message} onChange={handleInputChange} rows={5} maxLength={1000} disabled={formLoading} />
                  {formErrors.message && <span className="sp-field__err">{formErrors.message}</span>}
                </div>
                <div className="sp-form-actions">
                  <button type="submit" className="sp-btn-primary" disabled={formLoading}>
                    {formLoading ? <><Loader size={16} className="sp-spinner" /> Sending…</> : <><Send size={16} /> Send Message</>}
                  </button>
                </div>
              </motion.form>
            )}
      ...
      <AnimatePresence>
        {chatOpen && (
          <>
            <motion.div className="sp-chat-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeChat} />
            <motion.div className="sp-chat" initial={{ opacity: 0, y: 40, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 40, scale: 0.96 }} transition={{ type: 'spring', stiffness: 340, damping: 28 }}>
              <div className="sp-chat__header">
                <div className="sp-chat__agent">
                  ...
                  <div>
                    ...
                    <p className="sp-chat__agent-status"><span className="sp-chat__dot" /> Online · Support Agent</p>
                  </div>
                </div>
                <button className="sp-chat__close" onClick={closeChat}><X size={18} /></button>
              </div>
              <div className="sp-chat__body">
                {chatMessages.map((msg, i) => (
                  <motion.div key={i} className={`sp-chat__msg sp-chat__msg--${msg.from || msg.sender_type}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}>
                    <div className="sp-chat__bubble">{msg.text || msg.message}</div>
                  </motion.div>
                ))}
                {chatTyping && (
                  <div className="sp-chat__msg sp-chat__msg--agent">
                    <div className="sp-chat__bubble sp-chat__bubble--typing"><span /><span /><span /></div>
                  </div>
                )}
                <div ref={chatEndRef} />
              </div>
